(function () {
    ...
    function isPlaybackActionLabel(label) {
        return /(weiter\s*(?:ansehen|anschauen)|jetzt\s*(?:ansehen|anschauen)|fortsetzen|resume|continue\s*watching|watch\s*now|abspielen|anschauen|ansehen|play)/i
            .test(label || "");
    }
    ...
    function primaryDialogControl(dialog) {
        ...
        return controls.filter(function (control) {
            return isPlaybackActionLabel(controlLabel(control));
    ...
    function handleDetailDialogKey(event, dialog) {
        ...
        if (code === 13) {
            ...
            current.click();
            if (isPlaybackActionLabel(label)) {
    ...
    function expandLivePlayer(attempt) {
        ...
        var buttons = toArray(document.querySelectorAll(
            ".fullbrowser-btn-atc,.dockUp," +
            '[aria-label="Video erweitern"],[aria-label="Expand video"]'
        )).filter(function (button) {
            var rect = button.getBoundingClientRect();
            var style = window.getComputedStyle(button);
            return button.isConnected && rect.width > 20 && rect.height > 15 &&
                style.display !== "none" && style.visibility !== "hidden";
        });
        if (buttons.length) {
            buttons[0].click();
            console.log("[PlutoTV Navigation] expanded player via " +
                (buttons[0].getAttribute("aria-label") || buttons[0].className));
            return;
        }
        if (attempt < 20) {
            window.setTimeout(function () {
    ...
    function install() {
        ...
        console.log("[PlutoTV Navigation] installed 20260722-r7");
    }
